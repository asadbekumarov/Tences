...
async function promptStartGame(ctx) {
...
export function registerGroupQuizHandlers(bot) {
    ...
    bot.callbackQuery(/^gq_join:(.+)$/, async (ctx) => {
        ...
        const joined = await quizService.joinGame(gameId, ctx.from.id, ctx.from.first_name, ctx.from.username);
    ...
    bot.callbackQuery(/^gq_ans:([^:]+):(\d+):(\d+)$/, async (ctx) => {
        ...
        const { feedback, advance, answeredIndex } = await quizService.processAnswer(gameId, qIndex, optionId, ctx.from.id, ctx.callbackQuery.message?.message_id);
...
export function shutdownGroupQuiz() {
