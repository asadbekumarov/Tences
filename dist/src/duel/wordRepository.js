...
export async function ensureWordsForUnit(unit) {
...
export async function getWordCountForUnit(unit) {
    await ensureWordsForUnit(unit);
    return prisma.word.count({ where: { unit } });
}
export async function buildQuestionsForUnit(unit, options) {
    await ensureWordsForUnit(unit);
    ...
    return selected.map((word, index) => {
        const distractors = pickDistractors(words, word.translation, DUEL_CONFIG.OPTIONS_PER_QUESTION - 1);
        const optionTexts = shuffle([word.translation, ...distractors]);
        const options = optionTexts.map((text, optionId) => ({
            id: optionId,
            text,
