...
function formatQuestionText(word, prompt, index, total) {
    return (`❓ <b>Savol ${index + 1}/${total}</b>\n\n` +
        `<b>${escapeHtml(word)}</b>\n` +
        escapeHtml(prompt));
}
async function withTelegramRetry(fn, maxAttempts = 4) {
    let lastErr;
...
function formatFinalResults(players) {
...
export class GroupQuizService {
    api;
    lobbyEditChains = new Map();
    questionChains = new Map();
    constructor(api) {
        this.api = api;
    }
    runQuestionStep(gameId, fn) {
    ...
    clearQuestionChain(gameId) {
        this.questionChains.delete(gameId);
    }
    runLobbyEdit(gameId, fn) {
    ...
    async endDueToInactivity(gameId) {
        ...
        const results = board.length > 0 && board.some((p) => p.score > 0)
            ? formatFinalResults(board)
            : "Hech kim javob bermadi.";
        await this.editOrSend(live, `⏱ <b>2 daqiqa javob yo'q — o'yin tugadi</b>\n\n${results}`);
    }
    ...
    async startLobby(chatId, hostUserId, unit) {
    ...
    tickLobby(gameId) {
        const live = groupQuizManager.get(gameId);
        if (!live || live.status !== "LOBBY" || !live.messageId)
            return;
        const secondsLeft = Math.max(0, Math.ceil((live.lobbyEndsAt - Date.now()) / 1000));
        if (secondsLeft <= 0)
            return;
        const needsUpdate = live.lastLobbySecondsShown !== secondsLeft || live.lobbyPlayersDirty;
        if (!needsUpdate)
            return;
        this.runLobbyEdit(gameId, async () => {
            const liveNow = groupQuizManager.get(gameId);
            if (!liveNow || liveNow.status !== "LOBBY" || !liveNow.messageId)
                return;
            const secs = Math.max(0, Math.ceil((liveNow.lobbyEndsAt - Date.now()) / 1000));
            if (secs <= 0)
                return;
            try {
                await this.api.editMessageText(liveNow.chatId, liveNow.messageId, formatLobbyText(liveNow.unit, liveNow.totalQuestions, secs, liveNow.lobbyPlayersCache), {
                    parse_mode: "HTML",
    ...
    async joinGame(gameId, userId, firstName, username) {
        const live = groupQuizManager.get(gameId);
    ...
    async endLobby(gameId) {
        ...
        if (players.length < GROUP_QUIZ_CONFIG.MIN_PLAYERS) {
            ...
            groupQuizManager.remove(gameId);
            await this.editOrSend(live, "❌ O'yin bekor qilindi.\n\nHech kim qatnashmadi.", undefined);
            return;
        }
        try {
    ...
    async showQuestion(gameId, index) {
        const live = groupQuizManager.get(gameId);
        if (!live || live.status !== "ACTIVE")
            return;
        ...
        const text = formatQuestionText(question.word, question.prompt, index, live.totalQuestions);
        const displayed = await this.publishQuestion(live, text, createAnswerKeyboard(gameId, index, options));
        if (!displayed) {
    ...
    scheduleQuestionTimeout(gameId, index) {
        const live = groupQuizManager.get(gameId);
        if (!live)
            return;
        groupQuizManager.clearQuestionTimer(gameId);
    ...
    async processAnswer(gameId, questionIndex, optionId, userId, clickedMessageId) {
        const live = groupQuizManager.get(gameId);
        if (!live || live.status !== "ACTIVE") {
        ...
        if (!question)
            return { feedback: "Savol topilmadi", advance: false };
        ...
        if (!option)
            return { feedback: "Noto'g'ri variant", advance: false };
        if (!option.isCorrect) {
            this.resetInactivityTimer(gameId);
        ...
        groupQuizManager.clearQuestionTimer(gameId);
        this.resetInactivityTimer(gameId);
        const winText = `✅ <b>${escapeHtml(claim.firstName)}</b> birinchi topdi!\n\n` +
            `+1 ball ⚡`;
        const liveNow = groupQuizManager.get(gameId);
    ...
    async advanceQuestion(gameId, fromIndex) {
        return new Promise((resolve) => {
            this.runQuestionStep(gameId, async () => {
                const live = groupQuizManager.get(gameId);
                if (!live || live.status !== "ACTIVE")
                    return;
                if (live.currentIndex !== fromIndex)
                    return;
                groupQuizManager.clearQuestionTimer(gameId);
    ...
    async stripKeyboard(chatId, messageId) {
        if (!messageId)
            return;
        try {
    ...
    async publishQuestion(live, text, keyboard) {
        const oldMessageId = live.messageId;
        try {
            const msg = await withTelegramRetry(() => this.api.sendMessage(live.chatId, text, {
                parse_mode: "HTML",
    ...
    async editOrSend(live, text, keyboard) {
        const opts = {
            parse_mode: "HTML",
            reply_markup: keyboard,
        };
        ...
        const oldMessageId = live.messageId;
        try {
            const msg = await withTelegramRetry(() => this.api.sendMessage(live.chatId, text, opts));
            groupQuizManager.setMessageId(live.gameId, msg.message_id);
            await groupQuizRepository.setMessageId(live.gameId, msg.message_id);
            if (oldMessageId) {
                void this.api.deleteMessage(live.chatId, oldMessageId).catch(() => { });
