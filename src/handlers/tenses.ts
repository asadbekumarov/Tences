...
export function registerTenseHandlers(bot: Composer<Context>) {
  ...
  bot.callbackQuery(/^unit_(\d+)$/, async (ctx) => {
    ...
    try {
      ...
      const filteredWords = data.varaq1.filter((item: any) => item.unit === unitNumber);
      ...
      filteredWords.forEach((item: any, index: number) => {
