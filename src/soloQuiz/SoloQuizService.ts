...
import {
  createSoloAnswerKeyboard,
  createSoloUnitKeyboard,
  soloMenuKeyboard,
  soloPlayAgainKeyboard,
} from "./keyboard.js";
import { soloQuizManager } from "./SoloQuizManager.js";
import type { LiveSoloGame, SoloAnswerResult } from "./types.js";
...
function displayName(firstName: string, username?: string | null): string {
  const safe = escapeHtml(firstName);
  return username ? `@${escapeHtml(username)}` : safe;
}

function formatQuestion(game: LiveSoloGame): string {
  ...
  return (
    `🎮 <b>Unit ${game.unit}</b> | Ball: <b>${game.score}</b>\n\n` +
...
function formatSessionResult(game: LiveSoloGame): string {
  const total = game.questions.length;
...
async function upsertUser(ctx: SoloUserContext): Promise<void> {
...
export class SoloQuizService {
  ...
  async formatGlobalLeaderboard(currentUserId: number): Promise<string> {
    ...
    const myRank =
      ranked.findIndex((r) => r.user.id === toDbUserId(currentUserId)) + 1;
    const medals = ["🥇", "🥈", "🥉"];
  ...
  async showQuestion(sessionId: string): Promise<void> {
  ...
  async tickQuestion(sessionId: string): Promise<void> {
    const game = soloQuizManager.get(sessionId);
    if (!game || game.locked || !game.messageId) return;
    ...
    const elapsed = Math.max(
      0,
      SOLO_QUIZ_CONFIG.QUESTION_TIMEOUT_MS - (Date.now() - game.questionStartedAt),
    );
    const secLeft = Math.ceil(elapsed / 1000);
    if (secLeft <= 0) return;
    if (game.lastQuestionSecondsShown === secLeft) return;
  ...
  private async onTimeout(sessionId: string, questionIndex: number): Promise<void> {
    const game = soloQuizManager.get(sessionId);
    if (!game || game.locked || game.currentIndex !== questionIndex) return;
  ...
  processAnswer(
    ...
    if (game.userId !== userId) return { kind: "not_found" };
    if (game.currentIndex !== questionIndex) return { kind: "stale" };
    if (game.locked) return { kind: "stale" };

    const question = game.questions[questionIndex];
    const option = question.options.find((o) => o.id === optionId);
    if (!option) return { kind: "stale" };
  ...
  private async delayAndAdvance(sessionId: string, fromIndex: number): Promise<void> {
    await new Promise((r) => setTimeout(r, SOLO_QUIZ_CONFIG.ADVANCE_DELAY_MS));
    const game = soloQuizManager.get(sessionId);
    if (!game || game.currentIndex !== fromIndex) return;
  ...
  private async editOrSend(
    game: LiveSoloGame,
    text: string,
    keyboard?: ReturnType<typeof createSoloAnswerKeyboard> | ReturnType<typeof soloPlayAgainKeyboard>,
  ): Promise<number> {
